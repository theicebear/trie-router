...
Router.prototype.routes = function () {
  ...
  // router.route(paths).get(gen).post(gen)
  router.route = function () {
    return new Route(router, [].slice.call(arguments))
  }

  // router.get(paths, gen, gen ,gen)
  METHODS.forEach(function (method) {
    router[method.toLowerCase()] = function (paths) {
...
// For `router.route(paths).get(gen).post(gen)` support
METHODS.forEach(function (method) {
